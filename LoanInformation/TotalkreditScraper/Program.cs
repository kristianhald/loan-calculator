...
namespace TotalkreditScraper
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var exchangeList = httpClient.GetStringAsync("https://netbank.totalkredit.dk/netbank/showStockExchange.do").Result;
            ...
            foreach (var company in allCompanies)
            {
                ...
                if (name.Equals("TotalKredit", StringComparison.OrdinalIgnoreCase))
                    companyId = id;
            }

            var productTypes = JArray.Parse(
                httpClient.GetStringAsync($"http://configurationbutler-externalapi.azurewebsites.net/v0/loandata/201608221134/producttypes").Result); // Needs to be provided via configuration and via routing metadata

            var contributionRateStairCases = JArray.Parse(
                 httpClient.GetStringAsync($"http://configurationbutler-externalapi.azurewebsites.net/v0/loandata/201608221134/contributionratestaircase/{companyId}").Result); // Needs to be provided via configuration and via routing metadata);
